...
//                projectChild = withoutTargetDir.substring(1, secondSlash);
    ...
    function copyDependencyDirs(absoluteTargetDepUrl, copiedDirPathsMap){
        ...
        if(!namedPathUrlPathname){
            ...
            var projEquivPath = runtime.constructProjectPath(absoluteTargetDepUrl);
            ...
            var shouldCompile = projEquivPath.indexOf('.css') === (projEquivPath.length - 4) && !runtime.isExistingFilePath(projEquivPath) && runtime.isExistingFilePath(lessEquiv);
            console.log("Should compile " + absoluteTargetDepUrl + "? " + shouldCompile);
            if(shouldCompile){
...
//        return '<!doctype html>\n' + $("html")[0].outerHTML;
            ...
            console.error("Error while running modifyBuiltMarkupToRelativeWithJQuery", jqfe);
            console.trace(jqfe);
            throw jqfe;
        }
