...
function Project(args) {
    ...
    try{
        ...
        if(utils.hasPropertyOfType(pcfg, "theming", 'Object') && pcfg.theming.hasOwnProperty("enabled") && pcfg.theming.enabled === true){
            var varname = pcfg.theming.themeNameVar;
            var defaultTheme = pcfg.theming.defaultThemeName || pcfg.theming.themeNames[0];
...
Project.prototype.deleteIntermediaryFiles = function(){
    var jtp = this.listProjectJadeTemplatePaths();
    var deletedJadeFiles = jadeUtils.deleteCompiledFilesForTemplates(jtp);
    console.log("Deleted compiled JADE files ", deletedJadeFiles);
    var projDir = this.runtime.constructProjectPath("");
    ...
    var deletedCssFiles = lessCompiler.deleteAllCompiledCssFiles(lessFiles);
 ...
 * @return {String}
 */
Project.prototype.resolveProjectFile = function (pathName) {
 ...
 * @return {boolean}
 */
Project.prototype.projectPathExists = function (pathName) {
 ...
 * @param [args]
 */
Project.prototype.writeDynamicFile = function (fileName, content, args) {
 ...
 * @return {String}
 */
Project.prototype.readFile = function (projectPath) {
 ...
 * @param [args]
 */
Project.prototype.writeFile = function (projectPath, content, args) {
 ...
 * @return {boolean}
 */
Project.prototype.isHtmlFilename = function (fileName) {
 ...
 * @return {String[]}
 */
Project.prototype.listProjectTemplatePaths = function(){
    ...
    var lister = utils.createRecursiveDirLister({
        ...
        fileStatFilter: function (fileStat, filename, filepath) {
            return that.runtime.isProjectPath(filepath) && filepath.indexOf(bowerDirPath) !== 0 && !that.runtime.isNamedPathChild(filepath) && fileStat.isFile() && (that.isHtmlFilename(filename) || utils.endsWith(filename, ".jade") || utils.endsWith(filename, ".hbs")) && !that.isCompiledTemplateFilename(filename);
        }
    });
 ...
 * @return {String[]}
 */
Project.prototype.listProjectJadeTemplatePaths = function(){
    var dirPath = this.runtime.constructProjectPath("");
    var that = this;
    var lister = utils.createRecursiveDirLister({
        ignoredNames: ignoredNames,
 ...
 * @return {String[]}
 */
Project.prototype.listTemplatePaths = function (dirPath) {
    if (!dirPath) {
 ...
 * @return {String[]}
 */
Project.prototype.listAllTemplatePaths = function (dirPath) {
    ...
    var bu = new BowerUtils(dirPath);
    var bd = bu.getBowerDirectoryPath();
    var c;
    if(fs.existsSync(bd)){
        c = copier.listDirChildrenFullPathsRecursively(dirPath, [path.basename(bd)]);
    }else{
        c = copier.listDirChildrenFullPathsRecursively(dirPath);
    }

    var cs = utils.checksum(c.join());
 ...
 * @return {String}
 */
Project.prototype.toRelativePath = function (fullPath) {
 ...
 * @return {boolean}
 */
Project.prototype.isInSubdir = function (fullPath) {
 ...
 * @return {{}}
 */
Project.prototype.getUserConfig = function(){
 ...
 * @return {String[]}
 */
Project.prototype.listPathsWithReferences = function (dirPath, dropPointTypes) {
    if (!dirPath) {
        dirPath = this.runtime.constructProjectPath("");
    }
    var that = this;
 ...
 * @return {String[]}
 */
Project.prototype.listAllLayouts = function (dirPath) {
    if (!dirPath) {
        dirPath = this.runtime.constructProjectPath("");
    }
    var that = this;
...
Project.prototype.sortString = function (a, b) {
    if (a < b) {
        return -1;
    } else if (a > b) {
        return 1;
 ...
 * @param {String[]} paths
 */
Project.prototype.sortPathNames = function (paths) {
    var that = this;
 ...
 * @return {String[]}
 */
Project.prototype.listAllReferencedPaths = function (dirPath) {
    if (!dirPath) {
        dirPath = this.runtime.constructProjectPath("");
    }
    var that = this;
 ...
 * @return {boolean}
 */
Project.prototype.isCompiledTemplateFilename = function (fileName) {
 ...
 * @return {String}
 */
Project.prototype.toFileUrl = function (fullPath) {
 ...
 * @return {String[]}
 */
Project.prototype.listCompiledTemplatePaths = function (dirPath) {
    if (!dirPath) {
        dirPath = this.runtime.constructProjectPath("");
    }
    var that = this;
...
Project.prototype.updateDynamic = function () {
    ...
    var commandsJsonPath = path.resolve(dynamicDirPath, "projectCommands.json");
    if(!this.commandInfoWrittenOnce || !fs.existsSync(commandsJsonPath)){
        ...
        var modelPath = path.resolve(dynamicDirPath, "projectCommandsModel.json");
        var cmdPresModel = projectCommands.createCommandPresentationModel();
        fs.writeFileSync(modelPath, JSON.stringify(cmdPresModel));


        var tpl = fs.readFileSync(path.resolve(this.runtime.protostarDirPath, 'core','assets', 'menuTemplate-hb.html'), 'utf8');

        //var data = fs.readFileSync(this.runtime.constructProjectPath(["_dynamic", "list-compiled-bare.html"])'/home/spectre/Projects/IBM/theme-foundations/bootstrap/prototype/_dynamic/projectCommandsModel.json', 'utf8');
        //var jsonData = JSON.parse(data);
        //var obj = {
        //    data: jsonData
        //}
        //tpl = '{{#each data}}' + tpl + '{{/each}}';
        var compiled = Handlebars.compile(tpl);
        var markup = compiled(cmdPresModel);
        var commandsMenuMarkupPath = path.resolve(dynamicDirPath, "commandsMenu.html");
...
Project.prototype.isLibraryDirPresent = function(){
    var brcp = this.runtime.constructProjectPath('.bowerrc');
    if(fs.existsSync(brcp)){
        var brc = JSON.parse(this.runtime.readFile(brcp));
        if(brc.hasOwnProperty("directory")){
            var configuredLibDir = this.runtime.constructProjectPath(brc.directory);
            var exists = fs.existsSync(configuredLibDir);
...
Project.prototype.getLibraryDirPath = function(){
    var brcp = this.runtime.constructProjectPath('.bowerrc');
    if(fs.existsSync(brcp)){
        var brc = JSON.parse(this.runtime.readFile(brcp));
        if(brc.hasOwnProperty("directory")){
 ...
 */
Project.prototype.createProjectWcmMarkupInfo = function(){
    ...
    if(fs.existsSync(this.runtime.constructProjectPath('.bowerrc'))){
        var bowerRcData = JSON.parse(this.runtime.readFile(this.runtime.constructProjectPath('.bowerrc')));
        if(bowerRcData.hasOwnProperty("directory")){
            var depsDir = this.runtime.constructProjectPath(bowerRcData.directory);
            htmlFiles = htmlFiles.filter(function(f){
...
Project.prototype.logWcmMarkupFilesInProjectInfo = function(){
 ...
 * @return {String}
 */
Project.prototype.readViewScriptsMarkup = function readViewScriptsMarkup() {
