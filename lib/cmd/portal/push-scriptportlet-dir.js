...
module.exports=function (rc) {
    ...
    if(!fs.existsSync(spConfigPath)){
        ...
        console.error(msg);
        rc.composer.renderNewBackendView(msg, {
            title: 'Error - Missing sp-config - Protostar',
            pageTitle: 'Error',
            pageSubtitle: 'Missing sp-config.json'
        }, rc.response);
        return ;
    }
    ...
    if(missing.length > 0){
        ...
        console.error(msg);
        rc.composer.renderNewBackendView(msg, {
            title: 'Error - Missing properties in sp-config - Protostar',
            pageTitle: 'Error',
            pageSubtitle: 'Missing properties in sp-config.json'
        }, rc.response);
        return ;
    }
        ...
        blueBirdPromise.all(lessPromises).then(function(){
            ...
            exec(command, {
                ...
                if(error || (stdout+"").indexOf('Command was successful.  See the log for details') <0){
                    ...
                    opener = "Failed :-(";
                    msg = "There were errors while executing the push of <br><strong>" + dir + "</strong><br>initiated at " + start;
                    successful = false;
                }else{
                    console.info("Success: " + stdout);
                    opener = "Success!";
                    msg = "The compiled contents of <br><strong>" + dir + "</strong><br>were pushed to the live script portlet instance";
                    successful = true;
                }
                var logFilePath = path.resolve(tmpDir, "sp-cmdln.log");
                var out =
                    '<div class="row"><div class="col-md-12">' +
                        '<a href="/pscmds" class="btn btn-primary"><span class="glyphicon glyphicon-chevron-left"></span> Home</a>'+

                    '<p>'+msg+'</p>'+
                    '<p><a class="btn btn-primary" href="/?command=push-scriptportlet-dir&dir='+dir+'"><span class="glyphicon glyphicon-repeat"></span> Push '+dir +' again</a></p>' +
                    (stdout ? '<h3>Console Output</h3><pre><code>'+stdout+'</code></pre>' : '') +
                    (stderr ? '<h3>Console Error Output</h3><pre><code>'+stderr+'</code></pre>' : '') +
                    '<h3>Logfile <small>'+logFilePath+'</small></h3>'+
                    '<pre><code>'+utils.readTextFileSync(logFilePath)+'</code></pre>' +
                    '</div></div>';
