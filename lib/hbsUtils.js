...
hbsUtils.convertPartialsToFileIncludes = function(hbsSource, partialsDirProjectPath) {
    if(/\{\{>[ ]*body[ ]*}}/.test(hbsSource)){
        throw new Error("This fail contains a body injection point");
    }
    return hbsSource.replace(/{{>[ ]?/g, '<!-- file:' + partialsDirProjectPath + "/").replace(/[ ]?}}/g, ' -->');//.replace(new RegExp('<!-- file:' + partialsDirProjectPath + '/body -->','g'), '{{> body }}');
};
