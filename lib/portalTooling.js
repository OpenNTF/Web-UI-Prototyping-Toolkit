...
var runDxSyncCommand = function(dxSyncDir, cmd, dxsyncPath, cb){
    ...
    var np = process.argv[0];
    if(np.indexOf(" ")>0){
        np = '"' + np + '"';
    }
    var exec = require('child_process').exec;
    if(dxsyncPath.indexOf(' ') >0){
        dxsyncPath = '"' + dxsyncPath + '"';
    }
    var command = np + " " + dxsyncPath + " " + cmd;
...
portalTooling.isScriptPortletPushPathConfigured= function(project){
    var userCfg = project.runtime.readUserConfig();
    var configuredPath = userCfg["scriptPortletPushPath"];
    console.log("Configured script portlet path = " + configuredPath);
    return userCfg.hasOwnProperty("scriptPortletPushPath") && fs.existsSync(configuredPath);
};
