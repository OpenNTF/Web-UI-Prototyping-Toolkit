...
try{
    ...
    logger.info("jsdom node module is not installed, reverting to cheerio", jsdomErr.stack);
        ...
        jsdom.env({
            html: html, done: function (errors, window) {
                try {
                    ...
                    console.error("Could not create jquery instance", err.stack);
                    //console.trace(err);
                    callback(html, err);
                }
            }
    ...
    runCheerioFunction = function(html, jqueryFunction, callback, otherFnArg) {
        ...
        try{
            ...
            try {
                ...
                console.error("ERror from cheerio: ", CheerioFnErr.stack);
                throw CheerioFnErr;
            }finally {
                //window.close();
            }
        }catch(err){
            console.error("Could not create jquery instance : " + jqueryFunction.toString(), err.stack);
            //console.trace(err);
            callback(html, err);
        }
    };
