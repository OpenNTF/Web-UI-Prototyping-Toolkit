...
try{
    ...
    logger.info("jsdom node module is not installed, reverting to cheerio", jsdomErr);
...
if(jsdom) {
     ...
     * @param {String} html used to populate the window (aka the html to operate on with JQuery)
     * @param {Function} jqueryFunction the actual JQuery function($, window){}, will be executed async
     * @param {Function} callback the callback to run after the JQuery function has finished: function(result, errors, window){}
     * @param {Function} otherFnArg this will be passed to the JQUery function as 3rd argument after $ and window.
     ...
     */
    runJQuery = function (html, jqueryFunction, callback, otherFnArg) {
        windowIndex += 1;
        jsdom.env({
            html: html, done: function (errors, window) {
                try {
                    ...
                    logger.error("Could not create jquery instance", err.stack);
                    //console.trace(err);
                    callback(html, err);
                }
            }
    ...
    runCheerioFunction = function(html, jqueryFunction, callback, otherFnArg) {
        ...
        try{
            ...
            try {
                ...
                logger.error("ERror from cheerio: ", CheerioFnErr.stack);
                throw CheerioFnErr;
            }finally {
                //window.close();
            }
        }catch(err){
            logger.error("Could not create jquery instance : " + jqueryFunction.toString(), err.stack);
            //console.trace(err);
            callback(html, err);
        }
    };
    runJQuery = runCheerioFunction;
}
...
var ensureViewScriptsPresent = function($){
    ...
    //}
    if(!vw){
        //$('body').append('<script src="/ps/assets/runtimeActions.js" data-backend-only></script>');
        //$('body').append('<script src="/ps/assets/runtimeShortcuts.js" data-backend-only></script>');
        $('body').append('<script src="/ps/assets/uxFrame.js" data-backend-only></script>');
    }
};
...
var createPageRelativeReferences = function($, targetDir, metadata){
    ...
    var templateTargetPath = metadata.templateTargetPath;
    logger.debug("Ensuring relative refs in target page " + templateTargetPath + "  with source=" + templatePath);
    var config = {
        script:"src",
        link:"href",
        img:"src",
