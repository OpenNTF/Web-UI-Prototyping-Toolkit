...
function TemplateComposer(args) {
    ...
    this.findDropPoints = function(filepath, content, partType){
        return findDropPoints(filepath, content, partType);
    };
    ...
    function applyLoremPlaceholder(part, composed) {
        ...
        var replacement = "";
        var caseType = "lower";
        if(!utils.isDefined(part.args) || (utils.isArray(part.args)&& part.args.length < 1)){
            switch(part.name){
                case "word":
                ...
                case "phrase":
                    ...
                    break;
                default:
                    throw new Error("Unknown lorem invocation: " + part.name);
            }
        }else{
            //tag, separator, count, min, max
            var argVals = createArgsValsObject(part.args);
