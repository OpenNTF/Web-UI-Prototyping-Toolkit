...
function TemplateComposer(args) {
    ...
    linkScript: function(placeholder, composed, metadata, templateFilename){
    ...
    linkCss: function(placeholder, composed, metadata, templateFilename){
        var dirPath = path.dirname(templateFilename);
        var dirName = path.basename(dirPath);
        if (placeholder.isNamed('default')) {
    ...
    lorem: function(part, composed) {
        return loremFactory.applyPlaceholder(part, composed);
    },
    hb : function(partName, composed, metadata){
    ...
    wrap: function(partName, composed, metadata) {
        var wrapper;
    ...
    layout: function(layoutPlaceholder, composed, metadata) {
    ...
    function replacePartContents(content, part, partContents, addMarkers) {
        return part.replacePartContents(content, partContents, addMarkers);
    }
    ...
    function readFragment(filePath, metadata) {
        ...
        if (dropPoints.length > 0) {
            contents = placeholderProcessors.wrap(dropPoints[0], contents, metadata);
        }
    ...
    var compositionRun = function (templateFilename, template, placeholders, metadata) {
        ...
        placeholders.forEach(function (placeholder) {
            if(!placeholderProcessors.hasOwnProperty(placeholder.getType())){
                throw new Error("Unknown type " + placeholder.getType());
            }
            composed = (placeholderProcessors[placeholder.getType()])(placeholder, composed, metadata, templateFilename);
        });
        return composed;
    };
