...
function TemplateComposer(args) {
    ...
    function findDropPointsOfType(filepath, content, dropPointType) {
        ...
        while ((result = contents.indexOf(crit, startIdx)) >= 0) {
            ...
            } else if (type === "layout" || type === "wrap" || type === "hb") {
            ...
            if (utils.isRelativePath(dp.getName())) {
                ...
                switch(dp.getType()){
                    ...
                    case "linkScript":
                        ...
                        break;
                    default:throw new Error("unsupported default resource inclusion type (should be linkCss/linkScript)");
                }
    ...
    var replaceHandlebarsPlaceholder = function(partName, composed, metadata){
        //var name = partName.getName();
        //var parIdx = name.indexOf('(');
        //var nameOnly = name.substring(0, parIdx);
        var args = partName.getArgs();
        //var splitArgs = args.split(';');
        //var dataLocation = splitArgs[0];
        ...
        if(partName.getArgs().length === 1){
            ...
            //var compiledInstance = compiledTemplate(parsedData);
        ...
        if(partName.getArgs().length < 2){
            ...
            if(utils.isArray(parsedData)){
                var allCompiled = '';
                //var dereferencedArray = false;


                //parsedData.forEach(function(d){
                //    var nestedArgParts = partName.getArgs()[1].split('.');
                //    nestedArgParts.splice(0, 0, d);
                //    var exists = utils.nestedPathExists.call(this, nestedArgParts );
                //    if(exists){
                //        utils.isArray(utils.)
                //    }
                //});
    ...
    this.composeTemplateCached = function(filePath){
        ...
        console.log("COMPILATION RETURNED : ", composed);
        ...
        logger.debug("Cached compiled template : " + filePath);
        return composed;
    };
