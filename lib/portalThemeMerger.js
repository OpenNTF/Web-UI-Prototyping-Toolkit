...
function PortalThemeMerger(args) {
    ...
    this.isSimpleSelfContainedDir = function(dirPath){
        ...
        children.forEach(function(dn){
            ...
            if(fs.statSync(childPath).isDirectory()){
                ...
                try {
                    ...
                    console.log("Writing zip to " + targetZipPath);
                    zip.writeZip(targetZipPath);
                } catch (e) {
                    console.error("Could not create zip", e);
                    console.error(e.stack);
                    throw e;
                }

            }else{
                console.info("NOT a dir : " + childPath);
            }
        });
        console.log("EASY SCRIPT PORTLETS == ", easyScriptPortletComponents);

    };

    this.prepareComponentDir = function(cmpDir){
        console.info("PREP COMPONENT DIR = " + cmpDir);
        var that = this;
        ...
        removedIdxs.reverse();
        console.log("Removed paths ", removedIdxs);
        ...
        console.log("RELATIVE FILES == ", relativeFiles);
        ...
        files.html.forEach(function(htmlPath){
            console.log("Checking html " + htmlPath);
            ...
            allReferenceables.forEach(function(refPath){
                ...
                try {
                    ...
                    if (endIdx > 0) {
                        ...
                        var outHtml = "" + html;
                        console.log("NEED TO REPLACE " + toReplace + " with " + replacement);
                        var lastCritIdx = outHtml.lastIndexOf(toReplace);
                        while (lastCritIdx >= 0) {
                            var before = outHtml.substring(0, lastCritIdx);
                            console.log("BEFORE=" + before.substring(before.length - 5));
                            var after = outHtml.substring(lastCritIdx + toReplace.length);
                            console.log("AFTER=" + before.substring(0, 5));
                            outHtml = before + replacement + after;
                            lastCritIdx = outHtml.lastIndexOf(toReplace);
                        }
                        if (html !== outHtml) {
                            console.log("Updating " + htmlPath + " for " + refPath);
                            utils.writeFile(htmlPath, outHtml);
                        } else {
                            console.log("No need to update " + htmlPath + " for " + refPath);
                        }
                    }else{
                        console.info("There is no ref to " + refPath + " in " + htmlPath);
                    }
                } catch (e) {
        ...
        if(easy){
            ...
            if(relativeFiles.js.length === 1){
                ...
                initCnt = "" + cnt;
                read = true;
                if(cnt.indexOf(relativeFiles.js[0] + '"') < 0){
                    var src = relativeFiles.js[0];
            ...
            if(relativeFiles.css.length === 1){
                ...
                if(cnt.indexOf(relativeFiles.css[0] + '"') < 0){
                    var linktag = '<link rel="stylesheet" href="'+relativeFiles.css[0]+'"/>';
                    cnt = '\n'+linktag+'\n' + cnt;
                }
            }
        ...
        //var dirname = path.dirname(cmpDir);
        //var childDirNames = fs.readdirSync(cmpDir).filter(function(p){
        //    return fs.statSync(cmpDir + path.sep + p).isDirectory();
        //});
        //
        //childDirNames.forEach(function(cdn){
        //});
        return easy;
    ...
    this.createPackageZips = function(cmpDirsArray){
        ...
        return new Promise(function(resolve, reject){
            that.compileAllLessFilesPromise(cmpDirsArray).done(function(){
                ...
                //console.log("Creating component dirs for : ", value);

                cmpDirsArray.forEach(function(prjDirPath, idx){
                    var prefix = cmpDirsArray.length > 0 ? "dir_" + (idx+1) + "_" : "";
                    var cmpSourceDirPath = prjDirPath;//path.resolve(targetMergeDirPath, prjDirPath);
