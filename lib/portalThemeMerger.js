...
function PortalThemeMerger(args) {
    ...
    this.parseArgs = function(){
        console.info("PARSING MERGE ARGS: ", this.args);
    ...
    this.mergeProject = function(){
        ...
        return new Promise(function(mergeResolve, mergeReject){
            ...
            if(!rt.isExistingDirPath(pp)){
                ...
                mergeReject("There is no prototype at path " + pp);
                //errorCb("There is no prototype at path " + pp);
                return;
            }
            if(!rt.isExistingDirPath(tp)){
                console.error("There is no portal theme at path " + tp);
                mergeReject("There is no portal theme at path " + tp);
                //errorCb("There is no portal theme at path " + tp);
                return;
            }
            wrench.copyDirRecursive(pp, md, function(err){
                if(err){
                    console.error("Could not recursively copy " + pp + " -> " + md, err);
                    mergeReject(err);
                    return;
                }
                ...
                builder.buildPrototype().done(function(){
                    ...
                    copyMerge(buildDir, mergeDir).then(function(){
                        ...
                        return that.copyPaths(that.pathsToCopy);/*.then(function(){
                     ...
                     errorCb(errors);
                        ...
                        .then(function(){
                            console.log("after copy ops");
                            mergeResolve();
                        }).catch(function(){
                            console.error("error copy ops");
                            //errorCb();
                            mergeReject("error copy ops")
                        });
                    //Promise.join(copyOps, function(){
                    //
                    //    //successCb();
                    //}).catch(function(){
                    //
                    ...
                    mergeReject("BUILD ERROR " + errors);
                });
            });
        });
