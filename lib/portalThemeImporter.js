...
function ThemeImporter(args){
    ...
    this.convertToLayout = function(src){
    ...
    this.createNewThemeProject = function(projectName, dir, inputArgs, callback){
        ...
        wrench.mkdirSyncRecursive(path.join(staticFilesProjPath, "src/main"));
        wrench.mkdirSyncRecursive(path.join(dynamicFilesProjPath, "src/main"));
        wrench.mkdirSyncRecursive(path.join(earProjPath, "src/main/application"));
        ...
        this.downloadZip(themeFilesPath, function(){
            try{
                ...
                wrench.mkdirSyncRecursive(path.join(staticFilesProjPath, "src/main/webapp/themes"));
                ...
                wrench.copyDirRecursive(staticFilesSource, staticFilesTarget, function(){
                    ...
                    wrench.copyDirRecursive(dynaFilesSource, dynaFilesTarget, function(){
                        ...
                        wrench.mkdirSyncRecursive(path.join(staticFilesProjPath, srcWebInfPath));
                        ...
                        projectFiles.forEach(function(fp){
                            if(fp.substring(fp.length-5) === '.html' || fp.substring(fp.length-4) === '.jsp'){
                                var orig = fs.readFileSync(path.join(projRootPath, fp), {encoding: 'utf8'});
                                var modif = orig.replace(/85theme_/g, t.compiledArgs.projectName + "_");
                                fs.writeFileSync(path.join(projRootPath, fp), modif, {encoding: 'utf8'});
                ...
                console.error('error while creating theme', e);
                console.trace(e);
            }
        });
    };
