...
function ProtostarRuntime(args){
    ...
    this.parseArgs = function(args){
        if(!utils.isString(args.workingDir)){
            throw new Error("Illegal workingDirPath arg: " + args.workingDirPath);
        }
    ...
    this.readFile = function(fullPath){
        var now = "" + this.statFile(fullPath).mtime.getTime();
...
//            logger.info("Reading " + fullPath);
            ...
            readCache.pathContents[fullPath] = fs.readFileSync(fullPath) + "";
    ...
    this.writeFile = function(fullPath, contents, options){
        logger.info("writing " + fullPath + " : " + utils.formatByteSize(contents.length));
    ...
    this.readProjectConfig = function () {
        if (this.isProjectConfigPresent()) {
            ...
            logger.info("Read projectConfig from "+this.projectConfigPath);
            return   projectConfig;
        } else {
            logger.info("There is no project config at " + this.projectConfigPath);
            return {};
        }
    };
...
//                throw new Error("Uknown command: " + firstArg);
    ...
    this.readUserConfig = function(){
        ...
        try{
            ...
            console.trace(ParseException);
            logger.error("Invalid content in global config.json : " + configTxt, ParseException);
...
//        }
        ...
        if(!this.isExistingPath(filePath)){
            ...
            if(path.basename(path.dirname(filePath)) === '_dynamic'){
                console.log("Creating temp _dynamic file at " + filePath);
