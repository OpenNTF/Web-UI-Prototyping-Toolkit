...
//        }
        ...
        if(!this.isExistingPath(filePath)){
            console.log("Path doesnt exist : " + filePath);
            if(path.basename(path.dirname(filePath)) === '_dynamic'){
                ...
                console.log("Checkign jade path : " + jadeFp);
                if(fs.existsSync(jadeFp)){
                    var result = jadeUtils.jadeFileToHtmlFile(jadeFp);
                    var p = result.path;
            ...
            if(ext === '.html'){
                var jadeFp = filePath.substring(0, filePath.lastIndexOf('.')) + ".jade";
                if(fs.existsSync(jadeFp)){
                    console.log("Compiling jade fragment to be sure : " + jadeFp);
                    jadeUtils.jadeFileToHtmlFile(jadeFp);
                }
            }
        }
        return filePath;
