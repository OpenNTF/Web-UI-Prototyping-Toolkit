...
function getNestedPath(subject, pathPart1) {
    ...
    for (var i = 0; i < args.length; i++) {
        if (!object || !object.hasOwnProperty(args[i])) {
            console.error("Illegal args for neste dpath: ", args);
...
function Placeholder(args) {
    ...
    this.replacePartContents = function(content, partContents, addMarkers) {
        var am = false;
    ...
    this.replacePartContentsWithoutMarking = function(content, partContents) {
        return content.substring(0, this.getStart()) + partContents + content.substring(this.getEnd());
    }
...
module.exports = {
    ...
    countOwnProperties: function(obj){
        if(typeof obj !== 'object'){
            console.error("Passed instead of object: ", obj);
    ...
    parseNoArgsPlaceholder: function (tagName, type, startIdx, endIdx, fullTag, filepath) {
        ...
        return new Placeholder({
            name: nameOnly, start: startIdx, end: endIdx, type: 'file', tag: fullTag, filepath: filepath, args: args
        });
    }
};