...
function downloadJsonpData(options, cb){
    ...
    var req = http.request(opt, function(valResp){
        ...
        valResp.on('end', function () {
            var receivedStr = str;
            console.log("Finished jsonp request: ", str.substring(0, 200) + " ... ");
            try {
 ...
 */
var parsePlaceholder = function (currentName, fullTag, currentStartIndex, currentEndIndex, filepath) {

    var dropPointType = fullTag.substring(fullTag.indexOf(' ')+1, fullTag.indexOf(':'));

    var nameAndArgsPart = fullTag.substring(fullTag.indexOf(':')+1);

    var hasArgs = currentName.indexOf('(') > 1;
    if(hasArgs){
        var args = parseLayoutArgs(currentName);
        ...
        return new utils.Placeholder({
            name: fullTag.substring(currentName.indexOf(':')+1),
            start: currentStartIndex,
            end: currentEndIndex,
            type: dropPointType,
            tag: fullTag,
...
function parseDropPointArgs(dropPointTagName){

    var placeholderName = dropPointTagName;
    if(placeholderName.indexOf('(') < 0) return [];
...
// rather than just find the first occurrence
    ...
    parseDropPointArgs:parseDropPointArgs,
    /**
     *
    ...
    parseNoArgsPlaceholder: function (tagName, type, startIdx, endIdx, fullTag, filepath) {
        if(tagName.indexOf('(')>=0){
            throw new Error("Tagname shouldn't contain args: " + tagName);
        }
        if(fullTag.indexOf('<!-- '+type+':') !== 0)
            throw new Error("Parsing got type="+type+" but fulltag=" + fullTag);
        ...
        return new utils.Placeholder({
            name: nameOnly, start: startIdx, end: endIdx, type: type, tag: fullTag, filepath: filepath, args: args
        });
    },
