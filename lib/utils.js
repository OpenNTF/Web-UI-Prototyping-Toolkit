...
function downloadJsonpData(options, cb){
    ...
    var req = http.request(opt, function(valResp){
        ...
        valResp.on('end', function () {
            var receivedStr = str;
            logger.debug("Finished jsonp request :" + str.length + " chars", opt);
            try {
                ...
                logger.error("Response: " + receivedStr);
                cb(e);
            }
        });
    });
...
var findCssPreProcessorInfo = function(requestedFilePath, sourceFileExt){
    ...
    var map = false;
    var format;
    var filePath = requestedFilePath;

    if(endsWith(filePath, '.css.map')){
        var baseFromMap = filePath.substring(0, filePath.length - '.css.map'.length);
        ...
        var baseFromCss = filePath.substring(0, filePath.length - '.css'.length);
        ...
        var baseFromDeps = filePath.substring(0, filePath.length - ".css.deps.json".length);
...
function encryptDXSync( text ) {
    var cipher = crypto.createCipher( "aes-256-ctr", "U6Jv]H[tf;mxE}6t*PQz?j474A7T@Vx%gcVJA#2cr2GNh96ve+" );
...
function decryptDXSync( text ) {
    var decipher = crypto.createDecipher( "aes-256-ctr", "U6Jv]H[tf;mxE}6t*PQz?j474A7T@Vx%gcVJA#2cr2GNh96ve+" );
