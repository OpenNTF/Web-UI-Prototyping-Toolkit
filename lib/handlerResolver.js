        ...
        switch (request.method) {
            case 'GET':
                if (isCommandRequest(urlParts)) {
                    ...
                    if(urlParts.pathname === '/projectConfig'){
                        handlerName = "viewProjectConfig";
                    }else if(urlParts.pathname === '/pshelp'){
                        handlerName = "viewHelp";
                    }else if(urlParts.pathname === '/newPortalTheme'){
                        handlerName = "viewCreateNewPortalThemeMavenProjectForm";
                    }
                } else if(isDynamicDataUrl(request, response, urlParts)){
                    if(urlParts.pathname.indexOf("/ps/dynamic/deps/page/direct") === 0 && urlParts.query.hasOwnProperty("page")){
                        handlerName = "showDirectTemplateDependencies";
                    }else if(urlParts.pathname.indexOf("/ps/dynamic/deps/page/deep") === 0){
                        handlerName = "showDeepTemplateDependencies";
                    }else if (urlParts.pathname.indexOf("/ps/dynamic/images") === 0) {
                        handlerName = "handleImageListing";
                    }else if(urlParts.pathname.indexOf("/ps/dynamic/editdata") === 0){
                        handlerName = "handleEditData";
                    }else if(urlParts.pathname.indexOf("/ps/dynamic/commandNames") === 0){
                        handlerName = "commandNamesJSON";
                    }else{
                        handlerName = "unknownDynamicDataRequest";
                    }
                }
                if(urlParts.pathname.indexOf("/ps/buildTheme") === 0 && urlParts.query.hasOwnProperty("auth") && allowedThemeReqs.hasOwnProperty(urlParts.query.auth)){
                    handlerName = "handleCreateNewPortalMavenProjectZip";
                } else {
                    var requestedFilePath = runtime.findFileForUrlPathname(urlParts.pathname);
                    if (fs.existsSync(requestedFilePath)) {
                        interceptLessCssRequestParameters(request, response);
                        var stat = fs.statSync(requestedFilePath);

                        if (stat.isDirectory()) {
                            logger.debug("Deemed dir for redirect: pathName=" + urlParts.pathname + " filename=" + requestedFilePath);
                            handlerName = "handleRedirectToDirectoryIndex";
                            return handlerName;
                        }else{
                            if (stat.isFile()) {

                                if (utils.endsWith(urlParts.pathname, '.less')){
                                ...
                                } else if(urlParts.pathname.length >= 5 && (urlParts.pathname.toLowerCase().indexOf(".md") === urlParts.pathname.length-3)){
            ...
            case 'POST':
                ...
                if(urlParts.pathname.indexOf("/ps/buildTheme") === 0){
