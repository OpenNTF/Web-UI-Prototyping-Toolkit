                ...
                switch (request.method) {
                        ...
                        if(typeof urlParts.query.uri === 'string'){
                            ...
                            fs.stat(requestedFilePath).done(function(stat){
                                ...
                                if(urlPathExtname === '.html' || !urlPathExtname){
                                    ...
                                    var hbsFilePath = jadeFilePath.substring(0, jadeFilePath.lastIndexOf('.')) + ".hbs";
                                    ...
                                    var baseCssName = u.substring(0, u.lastIndexOf('.'));
                                    if(/-splitIE[0-9]*$/.test(baseCssName)){
                                        baseCssName = baseCssName.substring(0, baseCssName.lastIndexOf('-splitIE'));
                                    }
                                    var lessFp = runtime.findFileForUrlPathname(baseCssName + ".less");
                                    logger.debug("lessfp = " + lessFp);
                                    var scssFp = runtime.findFileForUrlPathname(baseCssName + ".scss");
                    ...
                    case 'POST':
                        if (isCommandRequest(urlParts)) {
                            handlerName = "commandRequest";
                        } else if(urlPathname.indexOf('/ps/grabImages') === 0){
                            handlerName = "grabPageImages";
                        }else if(urlPathname.indexOf("/ps/buildTheme") === 0){
                    ...
                    case 'PUT':
                        if (isCommandRequest(urlParts)) {
                            handlerName = "commandRequest";
                        } else if (urlPathname.indexOf("/ps/update/partsource") === 0) {
