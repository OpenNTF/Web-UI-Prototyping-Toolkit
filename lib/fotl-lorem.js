            ...
            if(argVals.hasOwnProperty("case")){
                switch(argVals.case){
                    case "upper":
                        ...
                        break;
                    default:
                        logger.error("Unknown case value for lorem case arg: ", argVals.case);
                        break;
                    case "lower":
                        caseType = "lower";
                        break;
...
function PlaceholderFactory(args){
    ...
    this.readLoremLines = function(){
        if(this.loremLines.length < 1){
            var loremTxt = this.runtime.readAppFile(["core", "assets", "lorem.txt"]);
    ...
    this.parsePlaceholder = function(tagName, fullTag, startIdx, endIdx, filepath){
        ...
        if(tagName.indexOf('(') >0){
            var loremName = tagName.substring(0, tagName.indexOf('('));
            var laremArgs = tagName.substring(tagName.indexOf('(')+1, tagName.lastIndexOf(')'));
            var loremArgsSplit = laremArgs.split(',');
            o = new Placeholder({
                name: loremName,
                start: startIdx,
                tag: fullTag,
