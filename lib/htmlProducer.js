...
function HtmlProducer(args) {
    ...
    function createListingMarkup(files) {
        var out = "<html><body><h1>ProtoStar Prototypes</h1><ul>";
        var that = this;
        ...
        out += '</ul><a href="?command=delete_compiled">Delete all compiled</a> <a href="?command=compile_all">Compile all</a></body></html>';
        return out;
    }

    this.createListingMarkup = createListingMarkup;
    function createBareListingEntriesMarkup(files) {
        var out = "";
        var that = this;
        files.forEach(function (filepath) {
            ...
            out += '<li>' + createMarkupLink("ps:" + templateName, displayName) + '</li>';
        });
        return out;
    };
    ...
    function createCompiledMarkup(files) {
        var out = "<html><body><h1>Compiled ProtoStar Prototypes</h1><ul>";
        var that = this;
        files.forEach(function (fd) {
            var file = fd; //fd.name;
