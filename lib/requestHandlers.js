...
function createHandlers(args) {
    ...
    var postProcessComposed = function (markup, callback) {
        if (markup.content.trim().length > 0) {
            ...
            console.log("Project Config = ", pcfg);
            ...
            console.log("Need to add view scripts? ", addViewScripts);
            var addDoctypeIfMissing = false;
            logger.trace("Running postProcessComposed for : ", markup);
    ...
    var handlers = {
        ...
        handleRestData: function (request, response) {
            ...
            var urlPathname = urlParts.pathname;
            console.log("REST " + request.method + " " + urlPathname);
            var modelPath;
            var fs = require("fs");
            var modelTypePrefix;
