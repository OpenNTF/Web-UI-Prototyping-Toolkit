...
//logger.info(marked('I am using __markdown__.'));

function ProtoStarServer(args) {
//    var lessParserAdditionalArgs = {
...
//                jqueryRunner.processProtostarAttributes($, runtime, metadata);
...
//        var partContents = composer.prepareEditableRefs(project.readFile(ur.partname + ".html");
        ...
        var storeFileContentUpdate = function ($, window) {
            ...
            var origId = ur.id;//ur.id.substring(0, ur.id.lastIndexOf("_"));
...
//        comp = utils.encodeHtmlEntities(comp);
                            ...
                            if (err) {
                                ...
                                if (stat.isDirectory()) {
                                    ...
                                    response.writeHead(200, {
                                        "Content-Type": mime.lookup(filename),
                                        "Server": "protostar",
                                        "Last-Modified": new Date(st.mtime).toString(),
    ...
    this.start = function () {
        logger.info("Starting Protostar v0.1.0");
        createServer();
