...
function ProtoStarServer(args) {
                        ...
                        if(lessPath){
                            if(runtime.isExistingFilePath(lessPath)){
                                if(map){
                                    ...
                                    sslc.handleCompileLessCssMap(lessPath, runtime.readFile(lessPath), response);
                                }else{
                                    sslc.handleCompileLessCss(lessPath, runtime.readFile(lessPath), response);
                            ...
                            if (err) {
                                ...
                                if (stat.isDirectory()) {
                                    ...
                                    fs.readFile(filename, undefined, function (err, file) {
                                        if (urlPathName.length < 4 || !nonResourceExt.hasOwnProperty(path.extname(urlPathName))) {
                                            if (urlPathName.lastIndexOf(".less") > 0 && urlQuery.hasOwnProperty("compile")) {
                                                //handleCompileLessCss(filename, file, response);
                                                sslc.handleCompileLessCss(filename, file, response);
                                            } else {
                                            ...
                                            if(urlQuery.hasOwnProperty("edit")){
                                                ...
                                                },markdownHelper.compileMarkdown(runtime.readFile(runtime.findFileForUrlPathname(urlPathName))));
                                            }else{
    ...
    createServer();
    var sslc = lessCompiler.createServerSideLessCompiler(runtime.constructProjectPath(""), project.lessParserAdditionalArgs);
}
