...
function ProtoStarServer(args) {
    ...
    var requestHandler = function (request, response) {
        resolveHandler(request, response).done(function(handlerName){
            console.info("handler="+handlerName);
            var handlerFn = handlers[handlerName];

            handlerFn(request, response);
        }, function(err){
            console.error("Could not reqsolve handler", err);
    ...
    function startServer() {
        ...
        if(runtime.isExistingProjectFilePath(["bower.json"])){
            ...
            bowerUtils.runBower(curProj, bowerExec, runtime.getNodeCommandPath()).done(function(){
                startListening();
            }, function(err){
                console.error("Bower check error", err);
            });
        }else{
            startListening();
        }
