...
function ProtoStarServer(args) {
    ...
    var handleBackendViewRequest = function(request, response, parsedUrl){
        ...
        }else if(parsedUrl.pathname === '/newPortalTheme'){
            var wrapper = '<!-- wrap:/ps/backend/layout-help -->\n';
    ...
    var handleDynamicDataRequest = function(request, response, parsedUrl){
        if(parsedUrl.pathname.indexOf("/ps/dynamic/deps/page/direct") === 0 && parsedUrl.query.hasOwnProperty("page")){

        }else if(parsedUrl.pathname.indexOf("/ps/dynamic/deps/page/deep") === 0){

        }else if (parsedUrl.pathname.indexOf("/ps/dynamic/images") === 0) {
    ...
    var handleExistingFileRequest = function(request, response, parsedUrl, requestedFilePath){
        fs.stat(requestedFilePath, function (err, stat) {
            ...
            if (stat.isDirectory()) {
                logger.debug("Deemed dir for redirect: pathName=" + parsedUrl.pathname + " filename=" + requestedFilePath);
    ...
    var requestHandleGET = function (request, response) {
        ...
        if(parsedUrl.pathname.indexOf("/ps/buildTheme") === 0 && parsedUrl.query.hasOwnProperty("auth") && allowedThemeReqs.hasOwnProperty(parsedUrl.query.auth)){
            //serve up
    ...
    var requestHandlePOST = function (request, response) {
        ...
        if(urlParts.pathname.indexOf("/ps/buildTheme") === 0){
            //if(urlParts.query.hasOwnProperty("auth") && allowedThemeReqs.hasOwnProperty(urlParts.query.auth)){
            //    //serve up
            //    var config = allowedThemeReqs[urlParts.query.auth];// JSON.parse(authorizedThemeReqData);
            //    var ti = portalThemeImporter.ThemeImporter(config);
            //    ti.createNewThemeProjectZipBuffer(config.projectName, os.tmpdir(), config, function(buffer){
            //        response.writeHead(200, {
            //            'Expires': 0,
            //            'Cache-Control': 'must-revalidate, post-check=0, pre-check=0',
