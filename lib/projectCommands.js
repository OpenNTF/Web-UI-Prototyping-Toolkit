...
function ProjectCommands(args) {
    ...
    var deleteIntermediaryFiles = function(){
    ...
    var imageCache;/* = new utils.ItemCache(function(image){
        var keyPart;
        try {
    ...
    var commandFactory = {
        ...
        "photos": function (request, response, project) {
            ...
            if(url_parts.query.hasOwnProperty("tags")){
                console.log("QUERY TAGS== ", url_parts.query.tags);
            ...
            if(cachedForTagsString.length > 0){
                //var cachedResp = remoteImageService.getCachedTaggedImageResponse(tagsString);
                //console.log("Cached PHOTOS request: ", cachedForTagsString.substring(0, 200) + " ... ");
                console.log("Writing from cache for " + tagsString);
                utils.writeResponse(response, 200, {
                    "Content-Type": "application/json; charset=utf-8"
            ...
            if(tagsString +"" === '[object,Arguments]') throw new Error();
            ...
            utils.downloadJsonpData(opt, function(err, data){
                ...
                console.log("Writing received for " + tagsString);
                utils.writeResponse(response, 200, {
                    "Content-Type": "application/json; charset=utf-8"
                }, str);
            });
        ...
        "dxsync-push" : function(request, response, project){
            var userCfg = runtime.readUserConfig();
        ...
        "retainUsedCssOnly": function (request, response, project) {
            var files = project.listAllTemplatePaths();
            var hp = createHtmlProducer();
            var out = hp.createListingMarkup(files) + project.readViewScriptsMarkup();
            composer.renderBackendView(request, response, out, 'cmdListAll.html');
        ...
        "retain-used-css": function (request, response, project) {
            var hostPrefix = 'http://localhost:' +runtime.port;
            var files = project.listAllTemplatePaths().map(function(f){
                return hostPrefix + runtime.createUrlPathForFile(f);
            });
    ...
    this.handleCommandRequest = function (command, request, response) {
        ...
        if(!commandFactory.hasOwnProperty(command)){
            throw new Error("unknown command: " + command);
        }
        var cmdFn = (commandFactory[command]);
        console.log("cmdFn=",cmdFn);
        var responseObj = cmdFn(request, response, project);
        if(runtime.isDebug()){
