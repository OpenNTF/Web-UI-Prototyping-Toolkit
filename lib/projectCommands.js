    ...
    var commandFactory = {
        ...
        "push-scriptportlet-dir" : function(request, response, project){
            ...
            lessFiles.forEach(function(lp){
                ...
                //lessCompiler.compilePromise(lp, [path.dirname(lp)], utils.readTextFileSync(lp), path.dirname(lp)).done(function(css){
                //    var cssPath = lp.substring(projectDir.length+1);
                //    cssPath = cssPath.substring(0, cssPath.lastIndexOf('.'));
                //    cssPath += '.css';
                //    utils.writeFile(path.resolve(tmpDir, cssPath), css.toString());
                //})
            });

            Promise.all(lessPromises).done(function(){

                console.log("Finished compiling to " + tmpDir);
                prepareComponentDir(tmpDir);
                var exec = require('child_process').exec;
                var spCommandPath = "/home/spectre/opt/sp_cmdln/sp.sh";
                //var spPortalPassword = "password";
                //var command = spCommandPath + " push -portalPassword " + spPortalPassword;
                ...
                exec(command, {
                    ...
                    var out = '<div class="row"><div class="col-md-12">' +
                            ...
                            '<p><a href="/?command=push-scriptportlet-dir&dir='+dir+'">Push '+dir +' again</a></p>' +
                            '<h3>stdout</h3>'+
            ...
            //console.log("Requested push of " + dir);
            //return {
            //    status: 302,
            //    headers: {
            //        "Location": "http://" + request.headers.host
