...
function ProjectCommands(args) {
    ...
    var taggedImageResponsesCache = {
        /*
        tagsString alphabetically = responseText
         */
    };

    var taggedImageResponsesLastSyncTimeStamp = 0;

    var emptyTaggedImageResponsesCache = function(){
        Object.keys(taggedImageResponsesCache).forEach(function(k){
            delete taggedImageResponsesCache[k];
        });
    };

    var isTaggedImageResponseCached = function(tags){
        return taggedImageResponsesCache.hasOwnProperty(createTagsString(tags));
    };

    var getCachedTaggedImageResponse = function(tags){
        return taggedImageResponsesCache[createTagsString(tags)];
    };

    var storeCachedImageResponse = function(tagsString, imageResponse){
        if(typeof imageResponse !== 'string') throw new Error("Unexpected image response: " + imageResponse);
        taggedImageResponsesCache[createTagsString(tagsString)] =imageResponse;
    };

    var createTagsString = function(tags){
        var arr;
    ...
    var commandFactory = {
        ...
        "photos": function (request, response, project) {
            ...
            function pushAllItemsInArray(dataString, array){
                var cachedData = JSON.parse(dataString);
                cachedData.items.forEach(function(i){
                    array.push(i);
                })
            }

            if(url_parts.query.hasOwnProperty("random_cached")){
                var k = Object.keys(taggedImageResponsesCache);
                if(k.length){
                    utils.writeResponse(response, 200, {
                        "Content-Type": "application/json; charset=utf-8"
                    }, getCachedTaggedImageResponse(k[Math.random()* k.length]));
                    return;
                }
            }else if(url_parts.query.hasOwnProperty("all_cached")){
                var all = [];
                for(var tags in taggedImageResponsesCache){
                    pushAllItemsInArray(taggedImageResponsesCache[tags], all);
                }
                shuffle(all);
                var out = {
                    items: all
            ...
            if(isTaggedImageResponseCached(tagsString)){
                console.log("Cached PHOTOS request: ", getCachedTaggedImageResponse(tagsString).substring(0, 200) + " ... ");
                utils.writeResponse(response, 200, {
                    "Content-Type": "application/json; charset=utf-8"
                }, getCachedTaggedImageResponse(tagsString));
                return;
            }
            ...
            http.request({
                host: "api.flickr.com",
                path: "/services/feeds/photos_public.gne" + "?tags="+tagsString+"&format="+format
            }, function(valResp){
                var str = '';
                ...
                valResp.on('end', function () {
                    console.log("Finished PHOTOS request: ", str.substring(0, 200) + " ... ");
                    str = str.replace(/_m.jpg"/g,'_b.jpg"').trim();
                    ...
                    if(str.length > 100){
                        storeCachedImageResponse(createTagsString(tagsString), str);
                    }
                    utils.writeResponse(response, 200, {
                        "Content-Type": "application/json; charset=utf-8"
                    }, str);
