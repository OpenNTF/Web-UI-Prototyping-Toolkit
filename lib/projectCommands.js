        ...
        "delete_compiled_jade" : function(request, response, project){
            ...
            jtp.forEach(function(tp){
                ...
                fs.stat(htmlEquiv).done(function(stat){
                    if(stat.isFile()){
                        logger.info("Deleting html for " + tp + " : " + htmlEquiv);
                        fs.unlink(htmlEquiv)
                    }
                }, function(err){
                    logger.info("JADE template is not compiled : " + tp);
                });
            });
            response.writeHead(200, {
                "Content-Type":"text/html; charset=utf8"
            });
            response.write('<p>This is the intentionally blank page after deleting all compiled JADE *.html files</p><p>Going <a href="/">Home</a> might trigger recompilation to HTML thus recreating the files :-)</p>')
            response.end();
        },
        "compile_all_jade" : function(request, response, project){
            var jtp = project.listProjectJadeTemplatePaths();
            jtp.forEach(function(tp){
                logger.info("Compiling JADE " + tp);
